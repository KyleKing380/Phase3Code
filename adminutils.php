...
//function for deleting a lawyer from the system
function deleteLawyerRecord()
{
    global $conn;
    $cid = $_POST['cid'];
    if (!empty($cid)){
        $sql = "delete from LAWYER where LAWYER_ID = '$cid'";
...
function showClientRecords()
{
    ...
    global $thisPHP;
     ...
     //shows the button for adding a new Lawyer
     //NOT IMPLIMENTED
    echo "<form id='insertForm' action='{$thisPHP}' method='post'>";
    echo "<fieldset><legend>Click to Insert New Lawyer to Database</legend>";
    echo "<input type='submit' name='showLawyerInsertForm' value='Add New Client'>";
    echo "</form></fieldset>";
    
    //displays a custom table of lawyer data in the database
    $sql = "SELECT LAWYER_ID, EMP_FNAME, EMP_LNAME, EMP_INIT, RETAINER_FEE, LAWYER_CATEGORY, FIRM_NAME FROM LAWYER, EMPLOYEE, LAWFIRM WHERE LAWYER.EMP_ID = EMPLOYEE.EMP_ID AND EMPLOYEE.FIRM_ID = LAWFIRM.FIRM_ID";
    ...
    {         
        echo "<h4>Lawyers</h4>";
        echo "<table>";
        echo "<thead><tr><td>Lawyer Id</td> <td>First Name </td> <td>Last Name</td><td>Middle Initial</td><td>Retianer Fee</td> ";
        echo "<td>Lawyer Category</td> <td>Firm Name</td> <td> Delete? </td></tr></thead>";   
        while($row = $result->fetch_assoc()) 
        {
            echo "<tbody><tr>";
            $cid = $row["LAWYER_ID"];
            echo  "<td>" . $cid . "  </td> <td> " . $row["EMP_FNAME"] . 
                  " </td> <td> " . $row["EMP_LNAME"] . 
                  " </td> <td> " . $row["EMP_INIT"] . 
                  " </td> <td> " . $row["RETAINER_FEE"] . 
    		      " </td> <td> " . $row["LAWYER_CATEGORY"] .
                  " </td> <td> " . $row["FIRM_NAME"] . 
                  "</td> <td> "; 
            ...
            echo "<input type='submit' name='btnLawyerDelete' value='Delete'></form>";
            echo "</td></tr></tbody>";
        }
    ...
    {         
        ...
        while($row = $caseresult->fetch_assoc()) 
        {
            ...
            echo "<form action='{$thisPHP}' method='post' style='display:inline' >";
            echo "<input type='hidden' name='cid' value='{$cid}'>";
            echo "<input type='submit' name='btnCaseDelete' value='Delete'></form>";
            echo "</td></tr></tbody>";
        }
